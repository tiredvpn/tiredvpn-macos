...
struct StatusPill: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 6)
        .background(
            Capsule().fill(Self.color(for: state).opacity(0.15))
        )
        .overlay(
            Capsule().stroke(Self.color(for: state), lineWidth: 1)
        )
    ...
    static func color(for state: TunnelState) -> Color {
        switch state {
        case .connected:              return Color.tvPrimary
        case .connecting, .reasserting: return Color.tvAccent
        case .disconnecting:          return Color.tvAccent.opacity(0.7)
        case .error:                  return Color.tvError
        case .disconnected:           return Color.tvTextSecondary
        }
    }
}
